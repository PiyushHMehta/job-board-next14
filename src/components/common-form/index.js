...
function CommonForm({ action, buttonText, isButtonDisabled, buttonType, formControls, formData, setFormData, handleFileChange }) {
    ...
    function renderInputByComponentType(getCurrentControl) {
        ...
        switch (getCurrentControl.componentType) {
            ...
            case 'file':
                content =
                    <Label for={getCurrentControl.name} className='flex bg-gray-100 items-center px-3 py-3 mx-auto mt-6 text-center border-2 rounded-lg cursor-pointer'>
                        <h2>{getCurrentControl.label}</h2>
                        <Input onChange={handleFileChange} id={getCurrentControl.name} type='file' className='ml-4' />
                    </Label>
                break;
